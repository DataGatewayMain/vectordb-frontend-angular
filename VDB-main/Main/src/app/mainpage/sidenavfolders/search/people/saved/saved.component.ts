...
export class SavedComponent {
  ...
  allData: any;
  updatedPids: any;
  constructor(private router: Router,private newService: NewService, private filterService: FilterService,private http:HttpClient,private authService: AuthService, private apiService: GetDataService, private getDataService: GetDataService, private dialog: MatDialog, private snackbar: MatSnackBar) {}
  ...
  search(): void {
    ...
    this.apiService.savedsearch(this.filters, this.currentPage).subscribe(
      (data: any) => {
        ...
        // Apply masking to each result
        this.results.forEach((item: { maskedEmail: string; email_address: string; }) => {
          item.maskedEmail = this.maskEmail(item.email_address);
        });
  
        // Check if each result has a matching pid and set a flag
        this.results.forEach((item: { hasUpdate: any; pid: any; }) => {
          item.hasUpdate = this.updatedPids.includes(item.pid); // Assuming 'pid' is a property of the item
        });
...
getDisplayedEmailAddress(item: any): string {
  ...
  exportToCSV(exportAll: boolean = false): void {
    ...
    if (exportAll) {
        ...
        this.fetchAllData().then((allData) => {
            console.log('Fetched all data:', allData); // Log fetched data
            if (allData && allData.length > 0) {
                this.isExporting = false; // Stop showing spinner
                this.exportData(allData); // Call exportData with all data
            } else {
                this.isExporting = false; // Stop showing spinner
        ...
        this.exportData(this.selectedRows); // Export selected rows
    }
}

  openExportDialogue(): void {
    
    console.log("Selected Rows:", this.selectedRows); // Debugging to ensure rows are selected
  ...
  //   const hasUpdates = this.selectedRows.some(row => !row.companyStatus);

  // if (hasUpdates) {
  //   this.snackbar.open('Please update the records before exporting them.', 'Close', {
  ...
  exportData(dataToExport: any[]): void {
    ...
    this.apiService.exportToCSV(filters, dataToExport).subscribe(
        async (data) => {
            ...
            // Send the totalExportedCount to the API
            const apikey = localStorage.getItem('api_key'); // Replace 'apikey' with the actual key name
            console.log(apikey);

            if (apikey) {
              // Store the export history with the CSV data
              ...
              this.http.post('http://192.168.0.7:3000/update-export-count', { api_key: apikey }).subscribe(
                response => {
                  console.log('Total count updated successfully:', response);
                },
                error => {
...
//  ?****************************
  ...
  saveData(): void {
    ...
    const selectedRowIds = this.selectedRows.map(row => row.id); // Assuming 'id' is the unique identifier
  
    this.apiService.saveDataToUserAccount1(selectedRowIds).subscribe(
      (response) => {
        console.log('Data saved successfully:', response);
        this.selectedRows = []; // Clear selected rows after saving
      },
      (error) => {
  ...
  exportAllData(): void {
    // Use allData to export
    const dataToExport = this.allData;
  
    // Check if dataToExport is empty
  ...
  private convertToCSV(data: any[]): string {
    ...
    const header = Object.keys(data[0]).join(','); // Assumes all objects have the same keys
  ...
  openButtonDialog(event: MouseEvent, item: any): void {
    this.selectedItemId = item.id; // Store the ID of the clicked item
    console.log('Opening dialog for item ID:', item
    );
